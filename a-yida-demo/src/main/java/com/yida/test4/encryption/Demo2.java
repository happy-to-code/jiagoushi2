package com.yida.test4.encryption;

import cn.hutool.core.util.CharsetUtil;
import cn.hutool.core.util.HexUtil;
import cn.hutool.core.util.StrUtil;
import cn.hutool.crypto.asymmetric.KeyType;
import cn.hutool.crypto.asymmetric.RSA;

/**
 * @author zhangyifei
 * @Date 2023/7/13 14:26
 * @Version 1.0
 */
public class Demo2 {
    public static void main(String[] args) {
        String PRIVATE_KEY = "";
        RSA rsa = new RSA(PRIVATE_KEY, null);

        String data = "adb123###阿第三方";

        byte[] encrypt2 = rsa.encrypt(StrUtil.bytes(data, CharsetUtil.CHARSET_UTF_8), KeyType.PrivateKey);
        String encodeHexStr = HexUtil.encodeHexStr(encrypt2);
        System.out.println("加密后16进制字符串 = " + encodeHexStr);

        System.out.println("-------------------------------------------------");
        String PUBLIC_KEY = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDmeMDh9vODsI3Uy+3eRNWtTJV0TDs/mjYqBWDbiKNg9bHcHGPRH0fwyHGH2kAdf9+W7gsPrxyB16pOef0UNx52ihKnvGL2TJWvJPaZrTlFEt8cWBU3wYczF+kr+Bb/mgLlgTccVs0PXgxfZO07PLUGgd8kKMLZ5HOGDRe8VLZozQIDAQAB";
        RSA rsa2 = new RSA(null, PUBLIC_KEY);

        byte[] decrypt2 = rsa2.decrypt(HexUtil.decodeHex(encodeHexStr), KeyType.PublicKey);
        String str = StrUtil.str(decrypt2, CharsetUtil.CHARSET_UTF_8);
        System.out.println("str = " + str);

    }
}

...
public class Test1 {
    ...
    public static void testSseCall() throws Exception {
        ...
        Request request = new Request.Builder().url(url).post(formBody).headers(Headers.of(headers)).build();
        OkHttpClient okHttpClient = new OkHttpClient.Builder().connectTimeout(3, TimeUnit.SECONDS).readTimeout(3, TimeUnit.MINUTES).build();
    ...
    private static String getToken() {
        try {
            Map<String, Object> resultMap = WuDaoUtils.getToken(TestConstants.AUTH_TOKEN_URL,
                    "2ad731a51e5d78d71a75db0ae0b02564.CUQBUw7aZu7UkDq2", TestConstants.PUBLIC_KEY);
            Double code = (Double) resultMap.get("code");
            if (code.intValue() == 200) {
